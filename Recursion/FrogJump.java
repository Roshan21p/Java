...
public class FrogJump {
    public static int bestPath(int[] h,int n,int idx){
        if(idx==n-1) return 0;
        int op1 = Math.abs(h[idx]-h[idx+1]) + bestPath(h,n,idx+1);
        if(idx==n-2) return op1;
        int op2 = Math.abs(h[idx]-h[idx+2]) + bestPath(h,n,idx+2);
        return Math.min(op1,op2);
    }
    public static void main(String[] args) {
        int[] h = {30,20,50,10,40};
        System.out.println(bestPath(h,h.length,0));
    }
}
